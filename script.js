document.addEventListener('DOMContentLoaded', () => {
    ...
    headerElement.addEventListener('click', () => {
        if (adminPanel.style.display === 'none') {
            ...
            if (clickCount >= REQUIRED_CLICKS) {
                ...
                console.log('YÖNETİCİ KONTROL PANELİ ETİNLEŞTİRİLDİ.');
                clickCount = 0; 
                
                // YÖNETİCİ MODU AKTİF EDİLDİĞİNDE DÜZENLEME MODUNU AÇ
                editMode = true;
    ...
    function renderScoreboard(isEditable) {
        ...
        for (let i = 0; i < half; i++) {
            ...
            // Satırın admin modunda olup olmadığını belirten sınıfı ekle (CSS'teki genişlikler için)
            if (isEditable) {
    ...
    dbRef.on('value', (snapshot) => {
        ...
        // editMode durumu, yöneticinin ekranında korunur.
        renderScoreboard(editMode);
    }, (error) => {
    ...
    toggleButton.addEventListener('click', () => {
        ...
        // Tabloyu yeni moda göre yeniden çiz
        renderScoreboard(editMode);
    });
    ...
    saveButton.addEventListener('click', () => {
        ...
        dbRef.set(scores)
            .then(() => {
                ...
                // Buradaki 'editMode = false;' satırı KALDIRILDI.
                // Yönetici, KAYIT SONRASI DÜZENLEME MODUNDA KALMAYA DEVAM EDER.
                renderScoreboard(editMode);
            })
            .catch((error) => {
