document.addEventListener('DOMContentLoaded', () => {
    ...
    const localStorageAdminKey = 'zeron_admin_active'; // LocalStorage anahtarı
    
    let clickCount = 0;
    let editMode = false;
    let activeEdit = null; // Güncelleme sırasında aktif olarak düzenlenen hücreyi tutar
     ...
     * @param {boolean} isPersistent - Yönetici yetkisinin LocalStorage'dan gelip gelmediğini belirtir.
     */
    function enableAdminMode(isPersistent = false) {
        ...
        // Eğer yetki tıklama ile alındıysa, kalıcı hale getir.
        if (!isPersistent) {
            localStorage.setItem(localStorageAdminKey, 'true');
        }
    }

    // --- Sayfa Yüklenince Kalıcı Yetki Kontrolü ---
    if (localStorage.getItem(localStorageAdminKey) === 'true') {
        // Yetki kalıcı, 50 tık beklemeden hemen aç.
    ...
    headerElement.addEventListener('click', () => {
        // Zaten kalıcı yetkili değilse veya panel görünmüyorsa tıkla
        if (adminPanel.style.display === 'none' && localStorage.getItem(localStorageAdminKey) !== 'true') {
            clickCount++;
     ...
     * @param {boolean} isEditable - Tablonun düzenlenebilir modda olup olmadığını belirtir.
     */
    ...
    // KRİTİK ADIM: Sayfa yüklenir yüklenmez varsayılan slotları göster.
    // Eğer kalıcı yetki varsa, editMode=true ile açılır.
    renderScoreboard(editMode); 
    console.log('Varsayılan 25 Slot başarıyla yüklendi. Şimdi Firebase verisi bekleniyor...');
    

    // FIREBASE OKUMA (CANLI DİNLEME) İŞLEVİ 
    dbRef.on('value', (snapshot) => {
        if (snapshot.exists()) {
            
            // --- 1. Aktif Düzenlemeyi Kaydet ---
            // Yönetici modu açıksa ve bir hücreye odaklanılmışsa, içeriğini geçici olarak kaydet.
            if (editMode) {
            ...
            console.warn("Firebase'de veri bulunamadı. Varsayılan slot verileri kullanılıyor.");
        }
        
        // Veri değiştikçe herkesin ekranını günceller
        renderScoreboard(editMode); 

        // --- 2. Kaydedilen Düzenlemeyi Geri Yükle ---
        // Yeniden çizim yapıldıktan sonra, düzenlenmekte olan içeriği geri yükle.
        if (activeEdit) {
            ...
            if (targetCell) {
                ...
                if (targetCell.childNodes.length > 0) {
                     range.setStart(targetCell.childNodes[0], targetCell.textContent.length);
                     range.collapse(true);
                     selection.removeAllRanges();
                     selection.addRange(range);
                }
            }
            activeEdit = null; // Geri yükleme yapıldı, sıfırla
        }
        
    }, (error) => {
        console.error("Firebase veri okuma hatası: ", error);
        console.error("Veri yüklenirken bir hata oluştu. Varsayılan slotlar gösteriliyor.");
        renderScoreboard(editMode); 
    });
    ...
    saveButton.addEventListener('click', () => {
        ...
        dbRef.set(scores)
            .then(() => {
                console.log('Veriler başarıyla kaydedildi ve canlı olarak güncellendi! Düzenleme modu açık kalıyor.');
                // Kayıt başarılı, tabloda son güncel veriyi göster (editMode true kalır)
                renderScoreboard(editMode);
            })
            .catch((error) => {
